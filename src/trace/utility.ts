...
export function mergeConfig(userConfig: TracerConfig): TracerConfig & {
   ...
   */
  if (
    target.spanLimits.attributeCountLimit === DEFAULT_ATTRIBUTE_COUNT_LIMIT &&
    target.generalLimits.attributeCountLimit !== DEFAULT_ATTRIBUTE_COUNT_LIMIT
  ) {
   ...
   */
  if (
    target.spanLimits.attributeValueLengthLimit === DEFAULT_ATTRIBUTE_VALUE_LENGTH_LIMIT &&
    target.generalLimits.attributeValueLengthLimit !== DEFAULT_ATTRIBUTE_VALUE_LENGTH_LIMIT
  ) {
...
export function attribeValueToString(value: SpanAttributeValue | undefined): string {
...
export const traceIdFakedButValid = 'TraceIdFakedValid012345678901234';
export const SpanIdFakedButValid = 'SpanIdFakedValid';
